...
def train(sess, dataloader, model):
    ...
    start_time = 0
    end_time = 0
    print('start testing time per step ...')
    print('warmup ...')
    # for _ in range(FLAGS.epoch):
    for x in dataloader.batch_iter(trainset, FLAGS.batch_size, True):
        global_steps+=1
        if global_steps == wormup+1:
            time.sleep(10)
            start_time = time.time()
            print('start testing ..')
        if global_steps == wormup+test_steps+1:
            end_time = time.time()
            print('time per step: ')
            print((end_time-start_time)/test_steps)
            break
        loss += model(x, sess)
        k += 1
...
if __name__=='__main__':
    ...
    print('='*30)
    print(gpu_device_name,tf.test.is_gpu_available())
    print('='*30)
