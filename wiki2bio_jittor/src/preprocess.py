...
def split_infobox():
    ...
    *.box.pos is the position counted from the begining of a field
    ...
    for k, m in tqdm(enumerate(mixb_word)):
        with open(bwfile[k], "w+") as h:
            for items in m:
                for sens in items:
                    h.write(str(sens) + " ")
                h.write('\n')
    for k, m in tqdm(enumerate(mixb_label)):
        with open(bffile[k], "w+") as h:
            for items in m:
                for sens in items:
                    h.write(str(sens) + " ")
                h.write('\n')
    for k, m in tqdm(enumerate(mixb_pos)):
...
def reverse_pos():
    ...
    for k, pos in tqdm(enumerate(bpfile)):
...
def check_generated_box():
    ...
    for case in [ftrain, ftest, fvalid]:
        ...
        for val, lab, pos, rpos in tqdm(zip(vals, labs, poses, rposes)):
...
def split_summary_for_rouge():
    ...
    for i, fi in tqdm(enumerate(bpfile)):
        fread = open(fi, 'r')
        k = 0
        for line in fread:
...
def table2id():
    ...
    vocab = Vocab()
    for k, ff in tqdm(enumerate(fvals)):
        fi = open(ff, 'r')
        ...
        fo.close()
    for k, ff in tqdm(enumerate(flabs)):
        fi = open(ff, 'r')
        ...
        fo.close()
    for k, ff in tqdm(enumerate(fsums)):
        fi = open(ff, 'r')
